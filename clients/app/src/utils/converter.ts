...
import crc from 'crc';
import { ALPHANUM_REGEX, CANISTER_MAX_LENGTH, SUB_ACCOUNT_BYTE_LENGTH } from './common/constants';
...
export const balanceToString = (balance: bigint, decimal = 8): BalanceString => {
  ...
  const formatBelowZeroRegexp = /(?:\.0*|(\.\d+?)0+)$/;
  return {
    total: aboveZero + '.' + belowZero,
    aboveZero,
    belowZero,
    formatAboveZero,
    formatTotal: `${formatAboveZero}.${belowZero}`.replace(formatBelowZeroRegexp, '$1').replace(/\.?0+$/, ''),
    formatTotalTo8: formatAboveZero + `.${Number(`0.${belowZero}`).toFixed(8).split('.')[1]}`,
    formatTotalTo6: formatAboveZero + `.${Number(`0.${belowZero}`).toFixed(6).split('.')[1]}`,
    formatTotalTo4: formatAboveZero + `.${Number(`0.${belowZero}`).toFixed(4).split('.')[1]}`,
    formatTotalTo2: formatAboveZero + `.${Number(`0.${belowZero}`).toFixed(2).split('.')[1]}`,
  };
};
