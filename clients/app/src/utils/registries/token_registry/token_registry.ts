...
export default {
  getTokenActor,
  getTokens,
  addToken: async ({ agent, tokenInfo }: any) => new TokenRegistry(agent).add(tokenInfo),
  // editToken: async ({ agent, tokenInfo }) => new TokenRegistry(agent).edit(tokenInfo),
  removeToken: async ({ agent, canisterId }:any) => new TokenRegistry(agent).remove(canisterId),
};
