...
import { BaseMethodsExtendedActor, BaseMethodsExtendedActor_ } from '../dab_utils/actorFactory';
import { SubAccount } from '../common/types';
...
interface AddedMehtodsToken {
  send: ({ to, from, amount, opts }: SendParams) => Promise<SendResponse>;
  getMetadata: () => Promise<Metadata>;
  getBalance: (user: Principal, subaccount?: [SubAccount]) => Promise<BalanceResponse>;
  burnXTC: ({ to, amount }: BurnParams) => Promise<BurnResult>;
  getDecimals: () => Promise<number>;
}

export type TokenServiceExtended<T> = BaseMethodsExtendedActor<T> & BaseMethodsExtendedActor_<T> & AddedMehtodsToken;

export interface InternalTokenMethods {
  send: (actor: ActorSubclass<any>, { to, from, amount }: SendParams, proxy?: boolean) => Promise<SendResponse>;
