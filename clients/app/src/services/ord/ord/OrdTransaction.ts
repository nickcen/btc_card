...
import { OrdCanisterSigner, toXOnly } from '../signer';
...
export function utxoToInput(utxo: UnspentOutput, publicKey: Buffer): TxInput | undefined {
...
export class OrdTransaction {
  ...
  private wallet: OrdCanisterSigner;
  private changedAddress?: string;
  private network: bitcoin.Network = bitcoin.networks.bitcoin;
  private feeRate: number;
  private pubkey: string;
  constructor(wallet: OrdCanisterSigner, network: bitcoin.Network, pubkey: string, feeRate?: number) {
    this.wallet = wallet;
  ...
  async createSignedPsbt() {
    ...
    psbt = await this.wallet.signPsbt(psbt);
