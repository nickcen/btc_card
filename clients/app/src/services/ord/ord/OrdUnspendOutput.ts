...
export class OrdUnspendOutput {
    ...
    for (let i = 0; i < ords.length; i++) {
      ...
      if (leftAmount - splitAmount)
        if (splitAmount > UTXO_DUST) {
          ...
          ordUnits.push(new OrdUnit(UTXO_DUST + splitAmount, [{ id, outputOffset: offset, unitOffset: 0 }]));
        }
   ...
   */
  getNonOrdSatoshis() {
    return this.ordUnits!.filter(v => v.ords.length == 0).reduce((pre, cur) => pre + cur.satoshis, 0);
  }
