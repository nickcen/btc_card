...
import { GetTransactionsResponse, Send, Timestamp } from 'srcPath/candid//nns-dapp';
import { NET_ID, ROSETTA_URL } from 'srcPath/utils/constants';
import { formatAssetBySymbol, parseBalance, TokenSymbol } from 'srcPath/utils/converter';
import { TransactionResponse } from '../ledgerConnection';
// import { NNSConnection as nns } from '../nnsConnection';
...
export const getICPTransactions = async (accountId: string): Promise<GetRossetaTransactionsResponse> => {
  ...
  //@ts-nocheck
  const transactionsInfo = transactions.map(({ transaction }:any) => getTransactionInfo(accountId, transaction));
  return {
    total: total_count,
